...
def get_files(root_path,cur_path, allow_types=[]):
    files = []
    for upf in models.UploadFileModel.objects.all():
        files.append({
            'url':upf.filepath,
            'mtime':0
        })
    ...
    # for item in items:
    #     item=unicode(item)
    #     item_fullname = os.path.join(root_path,'', item).replace("\\", "/")
    #     print 'item_fullname:',item_fullname
...
def UploadFile(request):
    ...
    OutputFile = str(uuid.uuid1()).replace('-','') + upload_original_ext
    OutputPathFormat = os.path.join(OutputPathFormat[:OutputPathFormat.rindex('/')],OutputFile).replace('\\','/')

    #所有检测完成后写入文件
    if state=="SUCCESS":
        if action=="uploadscrawl":
            state=save_scrawl_file(request,os.path.join(OutputPath,OutputFile).replace('\\','/'))
        else:
            #保存到文件中，如果保存错误，需要返回ERROR
            state=save_upload_file(file,os.path.join(OutputPath,OutputFile).replace('\\','/'))
    print 'filename:',upload_original_name
    print 'filepath:',urllib.basejoin(USettings.gSettings.MEDIA_URL , OutputPathFormat)
    print 'filetype:',upload_original_ext[1:]
    print 'filesize:',file.size
    upf  = models.UploadFileModel(filename=upload_original_name,filetype=upload_original_ext[1:],
                      filepath=urllib.basejoin(USettings.gSettings.MEDIA_URL , OutputPathFormat),
                      filesize=file.size)
    upf.save()
    #返回数据
    return_info = {
